...
(function () {
	...
	function getHeaderOffset() {
		var header = document.querySelector('.gt-header, .gt-retail-header.header, .header');
		if (!header) return 96;
	...
	function initIndustriesMobileStack(section) {
		...
		function syncStackHeight(afterLayout, forceRefresh) {
			...
			if (typeof ScrollTrigger !== 'undefined') {
				requestAnimationFrame(function () {
					if (!section.classList.contains('is-scrubbing') && !stackSyncLock) {
						stackSyncLock = true;
		...
		if (
			...
			scrollTriggerInstance = ScrollTrigger.create({
				...
				onRefresh: function () {
					if (stackSyncLock) return;
					stackSyncLock = true;
					measurePanelHeights();
					var refreshHeight = getVisibleStackDepth(total - 1) * DECK_STEP + maxPanelHeight + 24;
					stackHeightCache = refreshHeight;
					stackEl.style.minHeight = refreshHeight + 'px';
					stackEl.style.height = refreshHeight + 'px';
					stackSyncLock = false;
				},
			});
		}
		...
		window.addEventListener('resize', function () {
			...
			resizeTimer = window.setTimeout(function () {
				if (stackSyncLock || section.classList.contains('is-scrubbing')) return;
				measurePanelHeights();
				syncStackHeight();
			}, 150);
		}, { passive: true });
	...
	function initIndustriesScroll() {
		...
		function goToPage(index, immediate, fromTab) {
			if (!scrollTriggerInstance) return;
			index = Math.max(0, Math.min(total - 1, index));

			if (isPaging && !immediate) {
				cancelPendingPageScroll();
			}

			pageScrollGen += 1;
			var scrollGen = pageScrollGen;
			...
			var settled = false;
			function finish() {
				if (scrollGen !== pageScrollGen) return;
				if (settled) return;
				settled = true;
				if (pageSafetyTimer) {
					clearTimeout(pageSafetyTimer);
				...
				/* Cooldown only after a real page tween — not pin enter snap or tab clicks */
				if (!immediate && !fromTab) beginWheelLock();
			}
		...
		var tl = gsap.timeline({
			...
			scrollTrigger: {
				...
				onEnter: function () {
					if (window.__growteleScrollRestore) {
						return;
					}
					pinActive = true;
					isPaging = false;
				...
				onEnterBack: function () {
					if (window.__growteleGoHero || window.__growteleScrollRestore) {
						return;
					}
					pinActive = true;
					isPaging = false;
		...
		tabs.forEach(function (tab, index) {
			tab.addEventListener('click', function () {
				...
				ensureLenisRunning();
				pinActive = isPinLive();
				/* Immediate jump avoids Lenis tween stalls that soft-lock scroll on live */
				goToPage(index, true, true);
				requestAnimationFrame(function () {
					if (typeof ScrollTrigger !== 'undefined') {
						ScrollTrigger.refresh();
					}
