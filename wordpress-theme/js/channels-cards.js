/**
 * Channels cards — start as stacked fan, open to flat row at scroll level (one-shot)
 *
 * @package Growtele
 */
(function () {
	...
	function initChannelsCards() {
		...
		wrappers.forEach(function (wrapper) {
			...
			function tweenTo(stateKey, duration) {
				var pos = layout()[stateKey];
				return map.map(function (item) {
					item.el.dataset.layoutScale = String(pos[item.key].scale);
					return setCardProps(item.el, pos[item.key], true, duration);
				});
			}
			...
			/* Open = flat horizontal row (like design); closed = stacked fan */
			function openToRow() {
				if (isOpen) return;
				isOpen = true;
				...
				activeTweens = tweenTo('stack', 1.05);
			}
			...
			function resetToFan() {
				...
				activeTweens = tweenTo('fan', 0.65);
			}

			var openDelayCall = null;
			var HOLD_BEFORE_OPEN = 0.45;
			var PIN_SCROLL = 720;
			...
			function scheduleOpen() {
				cancelOpenDelay();
				openDelayCall = gsap.delayedCall(HOLD_BEFORE_OPEN, openToRow);
			}

			/* Start as stacked fan; pin section, pause, then open to flat row */
			applyState('fan');

			var sectionEl = wrapper.closest('.gt-channels');
			var headingEl = sectionEl ? sectionEl.querySelector('.gt-section-heading') : null;
			var triggerEl = sectionEl || headingEl || wrapper;

			var trigger = ScrollTrigger.create({
				trigger: triggerEl,
				start: 'top 18%',
				end: '+=' + PIN_SCROLL,
				pin: sectionEl || wrapper,
				pinSpacing: true,
				anticipatePin: 1,
				invalidateOnRefresh: true,
				onEnter: scheduleOpen,
				onEnterBack: scheduleOpen,
